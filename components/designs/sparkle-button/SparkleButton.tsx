...
// Source: https://codepen.io/jh3y/pen/LYJMPBL
...
export default function SparkleButton({ text = "Start the magic", onClick }: SparkleButtonProps) {
    const particleRefs = useRef<(SVGSVGElement | null)[]>([])

    const random = (min: number, max: number) => Math.floor(Math.random() * (max - min + 1) + min)

    useEffect(() => {
        // Ensure refs are set before applying styles
        const applyStyles = () => {
        ...
        // Apply styles immediately and also after a short delay to ensure DOM is ready
        applyStyles()
        ...
        <div className="sparkle-button">
            ...
            <span aria-hidden="true" className="particle-pen">
                {Array.from({ length: PARTICLE_COUNT }, (_, i) => (
                    ...
                    >
                        <path d={particlePath} fill="currentColor" stroke="none" />
                    </svg>
                ))}
            </span>
        </div>
